...
Both workflows are written and validated (`terraform fmt`/`validate` pass, all four Dockerfiles build, the frontend builds) but **not yet wired to a live GitHub repo** — this project hadn't been pushed to GitHub at the point CI/CD was added. Turning it on from a fresh clone:
...
3. Set `enable_github_oidc = true` and `github_repo = "<owner>/<repo>"` in `terraform.tfvars`, then `terraform apply` once more (locally) to create the deploy role.
4. Set the resulting `github_deploy_role_arn` output as the `AWS_DEPLOY_ROLE_ARN` repository variable in GitHub (Settings → Secrets and variables → Actions → Variables).
...
- [x] CI/CD (GitHub Actions) — written and validated (Terraform fmt/validate, Docker builds, frontend build all pass locally); not yet wired to a live repo since this project hasn't been pushed to GitHub — see the **CI/CD** section above for the exact activation steps
